...
db.run(
	`ALTER TABLE videos ADD COLUMN scanStatus TEXT DEFAULT NULL`,
	(err) => {
		// Ignore error if column already exists
		if (err && !err.message.includes("duplicate column")) {
			console.warn("⚠️ Could not add scanStatus column (may already exist):", err.message);
		}
	}
);
...
async function completeFullScanOnly(videoId, existingImageReasons) {
	return new Promise((resolve, reject) => {
		console.log(`🔄 Completing full scan for ${videoId} (quick scan already done)`);
		...
		const processAudioFull = () => {
			return new Promise((resolve) => {
				...
				try {
					...
					const audioOutput = execSync(`python whisper_scan_full.py`, {
						stdio: "pipe",
						timeout: 600000, // 10 minute timeout for full audio
						cwd: __dirname,
					}).toString();
					transcript = audioOutput;
					console.log(`📝 Full Audio AI output (first 200 chars):`, audioOutput.substring(0, 200));
				} catch (aiErr) {
					console.error("⚠️ Full Audio AI error:", aiErr.message);
					if (aiErr.stdout) {
						console.error("⚠️ Full Audio AI stdout:", aiErr.stdout.toString().substring(0, 200));
					}
		...
		// Process full audio
		processAudioFull().then((fullAudioResult) => {
			const fullAudioReasons = safeJsonParse(fullAudioResult, []);
			
			console.log(`📊 Full Audio scan: ${fullAudioReasons.length} flags`, fullAudioReasons);
			...
			if (allReasons.length > 0) {
				...
				console.log(`⚠️ Full scan found unsafe content for ${videoId}:`, allReasons);
				...
				db.run(
					...
					(err) => {
						if (err) {
							...
							console.log(`✅ Updated ${videoId} with final SAFE confirmation`);
			...
			console.error(`❌ Full scan completion failed for ${videoId}:`, err.message);
			reject(err);
		});
	});
}
...
app.post("/analyze", async (req, res) => {
	try {
		...
		// Validate input
		if (!videoId) {
			return res.status(400).json({ error: "Missing videoId", details: "Please provide a videoId in the request body" });
		}
		...
		// ✅ 1️⃣ CACHE CHECK
		db.get("SELECT * FROM videos WHERE videoId = ?", [videoId], async (err, row) => {
		...
		if (row) {
			try {
				...
				if (scanStatus === "quick" && row.safe === 1) {
					console.log(`🔄 Video ${videoId} has quick scan only, completing full scan...`);
					
				// Check if audio file still exists
				const audioPath = path.join(__dirname, "tmp", "audio.wav");
				if (!fs.existsSync(audioPath)) {
					console.log(`⚠️ Audio file not found, need to re-download for full scan`);
					// Will fall through to full scan below (will do Phase 1 + Phase 2)
				} else {
					// Audio exists, do full scan only (skip Phase 1)
					completeFullScanOnly(videoId, reasons).then((result) => {
		...
		try {
			...
			if (!audioDownloaded) {
				throw new Error(`Audio download failed: YouTube may be blocking downloads for this video. Try updating yt-dlp with: yt-dlp -U`);
			}
			...
			if (!videoDownloaded) {
				throw new Error(`Video download failed: YouTube may be blocking downloads for this video. Try updating yt-dlp with: yt-dlp -U`);
			}
			...
			try {
				...
				execSync(`ffmpeg -i tmp/preview.mp4 -vf "fps=1/10" -frames:v 50 tmp/frame_%03d.jpg -y`, {
					stdio: "inherit",
					timeout: 60000, // 1 minute timeout
				});
			} catch (ffmpegErr) {
				throw new Error(`Frame extraction failed: ${ffmpegErr.message || "Unknown error"}`);
			}

			// ✅ 5️⃣ & 6️⃣ PHASE 1: QUICK SCAN (Audio first 2min + Images in parallel)
			console.log("✅ Phase 1: Running quick scan (Audio first 2min + Images)...");
			...
			const processAudioQuick = () => {
				return new Promise((resolve) => {
					let transcript = "[]";
					try {
						const audioOutput = execSync(`python whisper_scan.py`, {
							stdio: "pipe",
							timeout: 120000, // 2 minute timeout
							cwd: __dirname,
						}).toString();
						transcript = audioOutput;
						console.log(`📝 Quick Audio AI output (first 200 chars):`, audioOutput.substring(0, 200));
					} catch (aiErr) {
						console.error("⚠️ Quick Audio AI error:", aiErr.message);
						if (aiErr.stdout) {
							console.error("⚠️ Quick Audio AI stdout:", aiErr.stdout.toString().substring(0, 200));
						}
						transcript = "[]"; // Default to empty if AI fails
					}
					resolve(transcript);
			...
			const processImages = () => {
				return new Promise((resolve) => {
					let imageResult = "[]";
					try {
						const output = execSync(`python image_scan.py`, {
							stdio: "pipe",
							timeout: 120000, // 2 minute timeout
							cwd: __dirname,
						}).toString();
						
						console.log(`📝 Image AI raw output (first 500 chars):`, output.substring(0, 500));
						...
						if (imageResult === "[]" && lines.length > 0) {
							console.warn("⚠️ No valid JSON found in image AI output. Last line:", lines[lines.length - 1]);
						}
					} catch (aiErr) {
						console.error("⚠️ Image AI error:", aiErr.message);
						if (aiErr.stdout) {
							console.error("⚠️ Image AI stdout:", aiErr.stdout.toString().substring(0, 500));
						}
			...
			console.log(`📊 Phase 1 - Quick Audio: ${quickAudioReasons.length} flags`, quickAudioReasons);
			console.log(`📊 Phase 1 - Images: ${imageReasons.length} flags`, imageReasons);
			...
			if (!phase1Safe) {
				console.log(`⚠️ UNSAFE CONTENT DETECTED in Phase 1:`, phase1Reasons);
			...
			console.log(`✅ Phase 1 complete: No unsafe content detected (preliminary)`);
			...
			processAudioFull().then((fullAudioResult) => {
				...
				console.log(`📊 Phase 2 - Full Audio: ${fullAudioReasons.length} flags`, fullAudioReasons);
				
				if (fullAudioReasons.length > 0) {
					// Full scan found unsafe content - update database
					console.log(`⚠️ Phase 2 found unsafe content for ${videoId}:`, fullAudioReasons);
					...
					console.log(`✅ Phase 2 complete: Full scan confirms SAFE for ${videoId}`);
				...
				console.error(`❌ Phase 2 background scan failed for ${videoId}:`, err.message);
			...
			if (e.message.includes("timed out") || e.message.includes("ETIMEDOUT")) {
				errorMessage = "Operation timed out";
				errorDetails = "The scan took too long to complete. The video may be very long or your connection is slow. Try a shorter video.";
			} else if (e.message.includes("download")) {
				errorMessage = "Download failed";
				errorDetails = "Failed to download video content. The video may be unavailable or private.";
			} else if (e.message.includes("ffmpeg") || e.message.includes("Frame extraction")) {
